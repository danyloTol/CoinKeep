...
const WidgetMTotalBalance = ({ thisMonthValue, lastMonthValue }: WidgetProps) => {
    ...
    return (
        <div className="flex flex-row justify-between items-center gap-4 py-2 px-6.25 outline-1 outline-text-secondary-light rounded-[30px] overflow-hidden">
            ...
            <div className='flex flex-col gap-2'>
                <div className={`flex flex-row items-center gap- rounded-full px-2 py-0.5 ${(typeof changePercent === 'number' && Number.isFinite(changePercent)) ? "bg-bg-success text-text-success" : "bg-bg-danger text-text-danger"}`}>
                ...
                <p className='text-text-secondary-light'>vs last month</p>
            </div>
        </div>
    )
}
